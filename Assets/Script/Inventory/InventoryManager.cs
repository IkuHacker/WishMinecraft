...
public class InventoryManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (!BlockUtilityManager.instance.IsAnyPanelOpen()) // Vérifie si aucun panneau n'est ouvert
            {
                ToggleInventory(); // Si aucun panneau n'est ouvert, ouvre/ferme l'inventaire
            }
            else
            {
                Debug.Log("Un panneau est déjà ouvert, impossible d'ouvrir l'inventaire.");
            }
        }
    ...
    public void SpawnItemInHand(ItemData item, bool isInCreative)
    {
        ...
        if (isInCreative){ newItem.count = newItem.item.maxStack; }
        else { newItem.count = 1;  }
